...
APP_DIR="/opt/bloodcloud-incus-bot"
BOT_FILE="$APP_DIR/bot.py"
SERVICE="bloodcloud-bot"

# ─────────────────────────────────────────────────────────────
# BloodCloud Incus VPS Bot Installer
# ─────────────────────────────────────────────────────────────
...
╔══════════════════════════════════════════════════════════╗
║                  BLOODCLOUD VPS BOT                     ║
║                 INCUS DEPLOYMENT SYSTEM                 ║
╚══════════════════════════════════════════════════════════╝
BANNER
printf '\033[0m'
echo

echo "[BloodCloud] Updating packages..."
apt-get update
apt-get install -y python3 python3-venv python3-pip incus sqlite3 iproute2 iptables dnsmasq-base

echo "[BloodCloud] Checking Incus..."
...
if ! incus version >/dev/null 2>&1; then
  echo
  echo "Incus is installed but not initialized."
  echo "Run: incus admin init"
  echo "Then run this installer again."
  exit 1
fi

echo "[BloodCloud] Creating bot directory..."
mkdir -p "$APP_DIR"
chmod 750 "$APP_DIR"

echo "[BloodCloud] Creating bot.py..."
cat > "$BOT_FILE" <<'BLOODCLOUD_BOT_PY'
import discord
...
# Container backend: Incus
CONTAINER_BACKEND = 'incus'
...
async def create_port_forward(user_id: str, container: str, vps_port: int, node_id: int) -> Optional[int]:
    ...
    if not host_port:
        return None
    try:
        await execute_incus(container, f"config device add {container} tcp_proxy_{host_port} proxy listen=tcp:0.0.0.0:{host_port} connect=tcp:127.0.0.1:{vps_port}", node_id=node_id)
        await execute_incus(container, f"config device add {container} udp_proxy_{host_port} proxy listen=udp:0.0.0.0:{host_port} connect=udp:127.0.0.1:{vps_port}", node_id=node_id)
        conn = get_db()
        cur = conn.cursor()
...
async def remove_port_forward(forward_id: int, is_admin: bool = False) -> tuple[bool, Optional[str]]:
    ...
    try:
        await execute_incus(container, f"config device remove {container} tcp_proxy_{host_port}", node_id=node_id)
        await execute_incus(container, f"config device remove {container} udp_proxy_{host_port}", node_id=node_id)
...
async def recreate_port_forwards(container_name: str) -> int:
    ...
    for row in rows:
        ...
        try:
            await execute_incus(container_name, f"config device add {container_name} tcp_proxy_{host_port} proxy listen=tcp:0.0.0.0:{host_port} connect=tcp:127.0.0.1:{vps_port}", node_id=node_id)
            await execute_incus(container_name, f"config device add {container_name} udp_proxy_{host_port} proxy listen=udp:0.0.0.0:{host_port} connect=udp:127.0.0.1:{vps_port}", node_id=node_id)
...
def sanitize_username_for_container(username: str) -> str:
    """
    Sanitize username for Incus container naming.
    Use safe alphanumeric/hyphen names for Incus containers.
    ...
    # Ensure it doesn't start or end with hyphen (container-name requirement)
    sanitized = sanitized.strip('-').lower()
    # Limit length to avoid issues (Incus container names have limits)
...
Subsystem sftp /usr/lib/openssh/sftp-server"""
        ...
        await execute_incus(container_name, 
...
# Incus command execution with multi-node support
async def execute_incus(container_name: str, command: str, timeout=120, node_id: Optional[int] = None):
    if node_id is None:
        node_id = find_node_id_for_container(container_name)
    ...
    if node['is_local']:
        try:
            ...
            if proc.returncode != 0:
                ...
                raise Exception(f"Local Incus command failed: {error}\nCommand: {full_command}")
            return stdout.decode().strip() if stdout else True
        except asyncio.TimeoutError as te:
            logger.error(f"Incus command timed out: {full_command} - {str(te)}")
            raise
        except Exception as e:
            logger.error(f"Incus Error: {full_command} - {str(e)}")
            raise
    else:
        ...
        try:
            ...
            if res.get("returncode", 1) != 0:
                ...
                raise Exception(f"Remote Incus command failed on {node['name']}: {stderr}\nCommand: {full_command}")
...
# Apply Incus config
async def apply_incus_config(container_name: str, node_id: int):
    try:
        await execute_incus(container_name, f"config set {container_name} security.nesting true", node_id=node_id)
        await execute_incus(container_name, f"config set {container_name} security.privileged true", node_id=node_id)
        await execute_incus(container_name, f"config set {container_name} security.syscalls.intercept.mknod true", node_id=node_id)
        await execute_incus(container_name, f"config set {container_name} security.syscalls.intercept.setxattr true", node_id=node_id)
        await execute_incus(container_name, f"config set {container_name} linux.kernel_modules overlay,loop,nf_nat,ip_tables,ip6_tables,netlink_diag,br_netfilter", node_id=node_id)
        try:
            await execute_incus(container_name, f"config device add {container_name} fuse unix-char path=/dev/fuse", node_id=node_id)
        except:
            pass
        raw_lxc_config = (
        ...
        await execute_incus(container_name, f"config set {container_name} raw.lxc '{raw_lxc_config}'", node_id=node_id)
        logger.info(f"Incus permissions applied to {container_name} on node {node_id}")
    except Exception as e:
        logger.error(f"Failed to apply Incus config to {container_name}: {e}")
...
async def apply_internal_permissions(container_name: str, node_id: int):
    try:
        ...
        for cmd in commands:
            try:
                await execute_incus(container_name, f"exec {container_name} -- bash -c \"{cmd}\"", node_id=node_id)
                ...
                if vps.get('expiration_date'):
                    ...
                    if days_remaining < 0:
                        if not vps.get('suspended', False):
                            try:
                                # Suspend the VPS
                                asyncio.run(execute_incus(container_name, f"stop {container_name}", node_id=node_id))
                                vps['status'] = 'stopped'
                                vps['suspended'] = True
...
async def get_container_cpu_pct_local(container_name: str):
    try:
        proc = await asyncio.create_subprocess_exec(
            "incus", "exec", container_name, "--", "top", "-bn1",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
        stdout, _ = await proc.communicate()
...
async def get_container_ram_local(container_name: str):
    try:
        proc = await asyncio.create_subprocess_exec(
            "incus", "exec", container_name, "--", "free", "-m",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
        stdout, _ = await proc.communicate()
...
async def get_container_disk_local(container_name: str):
    try:
        proc = await asyncio.create_subprocess_exec(
            "incus", "exec", container_name, "--", "df", "-h", "/",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
        stdout, _ = await proc.communicate()
...
async def get_container_uptime_local(container_name: str):
    try:
        proc = await asyncio.create_subprocess_exec(
            "incus", "exec", container_name, "--", "uptime",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
        stdout, _ = await proc.communicate()
...
async def get_container_networks(container_name: str, node_id: Optional[int] = None) -> Dict[str, str]:
    ...
    try:
        ...
        proc = await asyncio.create_subprocess_exec(
            "incus", "exec", container_name, "--", "ip", "addr", "show",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
...
def get_default_storage_pool():
    try:
        result = subprocess.run(['incus', 'storage', 'list', '--format', 'csv'], 
...
@bot.command(name='incus-list')
@is_admin()
async def incus_list(ctx, node_id: int = 1):
    try:
        result = await execute_incus("", "list", node_id=node_id)
        node = get_node(node_id)
        embed = create_info_embed(f"Incus Containers List on {node['name']}", result)
        await ctx.send(embed=embed)
    except Exception as e:
...
class OSSelectView(discord.ui.View):
    ...
    async def select_os(self, interaction: discord.Interaction):
        ...
        try:
            await execute_incus(container_name, f"init {os_version} {container_name} -s {DEFAULT_STORAGE_POOL}", node_id=self.node_id)
            await execute_incus(container_name, f"config set {container_name} limits.memory {ram_mb}MB", node_id=self.node_id)
            await execute_incus(container_name, f"config set {container_name} limits.cpu {self.cpu}", node_id=self.node_id)
            await execute_incus(container_name, f"config device set {container_name} root size={self.disk}GB", node_id=self.node_id)
            await apply_incus_config(container_name, self.node_id)
            await execute_incus(container_name, f"start {container_name}", node_id=self.node_id)
            ...
            if HOST_MOTD:
                try:
                    await execute_incus(container_name, f"exec {container_name} -- bash -c \"{HOST_MOTD}\"", node_id=self.node_id)
...
class ReinstallOSSelectView(discord.ui.View):
    ...
    async def select_os(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await execute_incus(self.container_name, f"init {os_version} {self.container_name} -s {DEFAULT_STORAGE_POOL}", node_id=self.node_id)
            await execute_incus(self.container_name, f"config set {self.container_name} limits.memory {ram_mb}MB", node_id=self.node_id)
            await execute_incus(self.container_name, f"config set {self.container_name} limits.cpu {self.cpu}", node_id=self.node_id)
            await execute_incus(self.container_name, f"config device set {self.container_name} root size={self.storage_gb}GB", node_id=self.node_id)
            await apply_incus_config(self.container_name, self.node_id)
            await execute_incus(self.container_name, f"start {self.container_name}", node_id=self.node_id)
            ...
            if HOST_MOTD:
                try:
                    await execute_incus(self.container_name, f"exec {self.container_name} -- bash -c \"{HOST_MOTD}\"", node_id=self.node_id)
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
            ...
            class ConfirmView(discord.ui.View):
                ...
                async def confirm(self, inter: discord.Interaction, item: discord.ui.Button):
                    ...
                    try:
                        ...
                        await execute_incus(self.container_name, f"delete {self.container_name} --force", node_id=self.node_id)
        ...
        if action == 'start':
            try:
                ...
                await execute_incus(container_name, f"start {container_name}", node_id=node_id)
                target_vps["status"] = "running"
                save_vps_data_immediate()
            ...
            try:
                ...
                await execute_incus(container_name, f"stop {container_name}", timeout=120, node_id=node_id)
            ...
            try:
                ...
                try:
                    await execute_incus(container_name, f"exec {container_name} -- which tmate", node_id=node_id)
                ...
                if not tmate_installed:
                    ...
                    try:
                        await execute_incus(container_name, f"exec {container_name} -- apt-get update -y", node_id=node_id)
                        await execute_incus(container_name, f"exec {container_name} -- apt-get install tmate -y", node_id=node_id)
                ...
                await execute_incus(container_name, f"exec {container_name} -- tmate -S /tmp/{session_name}.sock new-session -d", node_id=node_id)
                await asyncio.sleep(3)
                ssh_output = await execute_incus(container_name, f"exec {container_name} -- tmate -S /tmp/{session_name}.sock display -p '#{{tmate_ssh}}'", node_id=node_id)
...
async def delete_vps(ctx, user: discord.Member, vps_number: int, *, reason: str = "No reason"):
    ...
    try:
        await execute_incus(container_name, f"delete {container_name} --force", node_id=node_id)
...
async def add_resources(ctx, vps_id: str, ram: int = None, cpu: int = None, disk: int = None):
    ...
    if was_running:
        ...
        try:
            await execute_incus(vps_id, "stop {vps_id}", node_id=node_id)
    ...
    try:
        ...
        if ram is not None and ram > 0:
            ...
            await execute_incus(vps_id, f"config set {vps_id} limits.memory {ram_mb}MB", node_id=node_id)
            changes.append(f"RAM: +{ram}GB (New total: {new_ram_gb}GB)")
        if cpu is not None and cpu > 0:
            new_cpu += cpu
            await execute_incus(vps_id, f"config set {vps_id} limits.cpu {new_cpu}", node_id=node_id)
            changes.append(f"CPU: +{cpu} cores (New total: {new_cpu} cores)")
        if disk is not None and disk > 0:
            new_disk_gb += disk
            await execute_incus(vps_id, f"config device set {vps_id} root size={new_disk_gb}GB", node_id=node_id)
        ...
        save_vps_data()
        if was_running:
            await execute_incus(vps_id, f"start {vps_id}", node_id=node_id)
...
async def restart_vps(ctx, container_name: str):
    ...
    try:
        await execute_incus(container_name, f"restart {container_name}", node_id=node_id)
        for user_id, vps_list in vps_data.items():
            for vps in vps_list:
                if vps['container_name'] == container_name:
                    vps['status'] = 'running'
...
async def execute_command(ctx, container_name: str, *, command: str):
    ...
    try:
        output = await execute_incus(container_name, f"exec {container_name} -- bash -c \"{command}\"", node_id=node_id)
            ...
            try:
                ...
                for node in nodes:
                    if node['is_local']:
                        ...
                        url = f"{node['url']}/api/execute"
                        data = {"command": "incus stop --all --force"}
                        params = {"api_key": node["api_key"]}
...
async def resize_vps(ctx, container_name: str, ram: int = None, cpu: int = None, disk: int = None):
    ...
    if was_running:
        ...
        try:
            await execute_incus(container_name, f"stop {container_name}", node_id=node_id)
            found_vps['status'] = 'stopped'
            save_vps_data()
        except Exception as e:
            await ctx.send(embed=create_error_embed("Stop Failed", f"Error stopping VPS: {str(e)}"))
    ...
    try:
        ...
        if ram is not None and ram > 0:
            new_ram = ram
            ram_mb = ram * 1024
            await execute_incus(container_name, f"config set {container_name} limits.memory {ram_mb}MB", node_id=node_id)
            changes.append(f"RAM: {ram}GB")
        if cpu is not None and cpu > 0:
            new_cpu = cpu
            await execute_incus(container_name, f"config set {container_name} limits.cpu {cpu}", node_id=node_id)
            changes.append(f"CPU: {cpu} cores")
        if disk is not None and disk > 0:
            new_disk = disk
            await execute_incus(container_name, f"config device set {container_name} root size={disk}GB", node_id=node_id)
...
async def clone_vps(ctx, container_name: str, new_name: str = None):
    ...
    try:
        ...
        if not found_vps:
            await ctx.send(embed=create_error_embed("VPS Not Found", f"No VPS found with container name: `{container_name}`"))
            return
        await execute_incus(container_name, f"copy {container_name} {new_name}", node_id=node_id)
        await apply_incus_config(new_name, node_id)
        await execute_incus(new_name, f"start {new_name}", node_id=node_id)
...
async def migrate_vps(ctx, container_name: str, target_node_id: int):
    ...
    try:
        ...
        await execute_incus(container_name, f"copy {container_name} {temp_name} -s {DEFAULT_STORAGE_POOL}", node_id=target_node_id)
        await execute_incus(container_name, f"delete {container_name} --force", node_id=node_id)
        await execute_incus(temp_name, f"rename {temp_name} {container_name}", node_id=target_node_id)
        await apply_incus_config(container_name, target_node_id)
        await execute_incus(container_name, f"start {container_name}", node_id=target_node_id)
...
async def node_check(ctx, node_id: int):
    ...
    if status.startswith("🟢"):
        ...
        try:
            pools_output = await execute_incus("", "storage list", node_id=node_id, timeout=30)
            add_field(embed, "💾 Available Storage Pools", f"```{pools_output}```", False)
            
            # Try to get default profile
            try:
                profile_output = await execute_incus("", "profile list", node_id=node_id, timeout=30)
...
async def vps_network(ctx, container_name: str, action: str, value: str = None):
    ...
    try:
        if action.lower() == "list":
            output = await execute_incus(container_name, f"exec {container_name} -- ip addr", node_id=node_id)
            if len(output) > 1000:
                output = output[:1000] + "\n... (truncated)"
            ...
            await execute_incus(container_name, f"config device set {container_name} eth0 limits.egress {value}", node_id=node_id)
            await execute_incus(container_name, f"config device set {container_name} eth0 limits.ingress {value}", node_id=node_id)
            ...
            await execute_incus(container_name, f"config device add {container_name} eth1 nic nictype=bridged parent={value}", node_id=node_id)
            ...
            await execute_incus(container_name, f"config device remove {container_name} {value}", node_id=node_id)
...
async def vps_processes(ctx, container_name: str):
    ...
    try:
        output = await execute_incus(container_name, f"exec {container_name} -- ps aux", node_id=node_id)
        if len(output) > 1000:
            output = output[:1000] + "\n... (truncated)"
...
async def vps_logs(ctx, container_name: str, lines: int = 50):
    ...
    try:
        output = await execute_incus(container_name, f"exec {container_name} -- journalctl -n {lines}", node_id=node_id)
        if len(output) > 1000:
            output = output[:1000] + "\n... (truncated)"
...
async def apply_permissions(ctx, container_name: str):
    ...
    try:
        ...
        if was_running:
            await execute_incus(container_name, f"stop {container_name}", node_id=node_id)
        await apply_incus_config(container_name, node_id)
        await execute_incus(container_name, f"start {container_name}", node_id=node_id)
        await apply_internal_permissions(container_name, node_id)
        await recreate_port_forwards(container_name)
        for user_id, vps_list in vps_data.items():
        ...
        for vps in vps_list:
            if vps.get('status') == 'running' and not vps.get('suspended', False) and not vps.get('whitelisted', False):
                ...
                if cpu > CPU_THRESHOLD or ram > RAM_THRESHOLD:
                    ...
                    try:
                        await execute_incus(container, f"stop {container}", node_id=node_id)
                        vps['status'] = 'stopped'
                        vps['suspended'] = True
                        if 'suspension_history' not in vps:
                            vps['suspension_history'] = []
        ...
        self.command_categories = {
            ...
            "nodes": {
                ...
                "commands": [
                    ...
                    (f"{PREFIX}incus-list [node_id]", "List Incus containers on node (Admin only)")
                ],
                "admin_only": True
            },
            "bot": {
            ...
            "admin": {
                ...
                "commands": [
                    (f"{PREFIX}incus-list", "List all Incus containers"),
...
echo "[BloodCloud] Creating Python environment..."
python3 -m venv "$APP_DIR/venv"
"$APP_DIR/venv/bin/python" -m pip install --upgrade pip setuptools wheel

echo "[BloodCloud] Installing Python packages..."
...
if [[ ! -f "$APP_DIR/.env" ]]; then
  cat > "$APP_DIR/.env" <<'ENV'
# ═════════════════════════════════════════════
# BloodCloud Incus VPS Bot Configuration
# ═════════════════════════════════════════════
...
# Add your existing bot variables below if needed.
ENV
  chmod 600 "$APP_DIR/.env"
fi

echo "[BloodCloud] Creating systemd service..."
cat > "/etc/systemd/system/${SERVICE}.service" <<EOF
[Unit]
Description=BloodCloud Incus VPS Discord Bot
...
ExecStart=${APP_DIR}/venv/bin/python ${BOT_FILE}
Restart=always
RestartSec=5
User=root
...
echo "[BloodCloud] Installing terminal MOTD..."
cat > /etc/motd <<'MOTD'
...
║                 INCUS HOSTING NODE                      ║
╠══════════════════════════════════════════════════════════╣
║  Status : \033[1;32mONLINE\033[1;31m                                      ║
║  Engine : \033[1;37mIncus\033[1;31m                                       ║
║  Bot    : \033[1;37mBloodCloud VPS Bot\033[1;31m                         ║
╚══════════════════════════════════════════════════════════╝\033[0m

\033[1;31mBloodCloud Commands\033[0m
  systemctl status bloodcloud-bot
  systemctl restart bloodcloud-bot
  systemctl stop bloodcloud-bot
  journalctl -u bloodcloud-bot -f

  incus list
  incus info <container>
  incus exec <container> -- bash

\033[1;31mBloodCloud • Incus VPS Infrastructure\033[0m
...
echo
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo " BloodCloud installation complete."
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo
echo "Bot:     $BOT_FILE"
